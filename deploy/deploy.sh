...
if command -v cross &>/dev/null; then
    (cd "$REPO_ROOT" && cross build --release --target "$TARGET" -p sin-server -p sin-cli)
else
    (cd "$REPO_ROOT" && cargo build --release --target "$TARGET" -p sin-server -p sin-cli)
fi

echo "==> Uploading binaries"
rsync -az --progress \
    "$RELEASE_DIR/sin-server" \
    "$RELEASE_DIR/sin" \
    "$PI:/tmp/"
...
set -euo pipefail
sudo mv /tmp/sin-server /usr/local/bin/sin-server
sudo mv /tmp/sin      /usr/local/bin/sin
sudo chmod +x /usr/local/bin/sin-server /usr/local/bin/sin
sudo chown root:root  /usr/local/bin/sin-server /usr/local/bin/sin
