...
build-pi:
	@if command -v cross >/dev/null 2>&1; then \
		cross build --release --target $(TARGET) -p rf-socket -p sin-cli; \
	else \
		cargo build --release --target $(TARGET) -p rf-socket -p sin-cli; \
	fi
...
.PHONY: build
build:
	cargo build --release -p rf-socket -p sin-cli

# Run the demo server locally for development
.PHONY: dev
dev:
	SIN_BASE=http://localhost:8080 cargo run -p rf-socket
